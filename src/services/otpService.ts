...
export const requestEmailOtp = (email: string, purpose: OtpPurpose) =>
    requestOtpCallable({ email: email.trim(), purpose });
...
export const verifyPasswordResetOtp = (email: string, otp: string) =>
    verifyResetCallable({ email: email.trim(), otp }).then(
        (result) => result.data,
    );
export const completePasswordReset = (
    ...
    completeResetCallable({ email: email.trim(), resetTicket, password }).then(
        (result) => result.data,
    );
