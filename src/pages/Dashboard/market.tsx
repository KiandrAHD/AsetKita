...
export default function Market() {
    ...
    return (
        <PageFrame
            title="Market"
            description="Harga simulasi diperbarui otomatis setiap 4 jam."
        >
                ...
                <div className="flex gap-2 overflow-x-auto">
                    {(["all", "saham", "kripto", "logam"] as const).map((item) => (
                        <button
                            ...
                            className={`rounded-xl px-4 py-2 text-sm capitalize ${category === item ? "bg-cyan-400 text-slate-950" : "bg-white/5 text-slate-300"}`}
                        >
                            {item === "all" ? "Semua" : item}
                        </button>
                    ))}
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#101b2a]">
                <div className="grid grid-cols-[1fr_auto_auto] gap-4 border-b border-white/10 px-4 py-3 text-xs text-slate-400 sm:grid-cols-[1fr_150px_110px_80px]">
                    <span>Aset</span>
                    <span>Harga</span>
                    <span>24 jam</span>
                ...
                {rows.map((asset) => {
                    ...
                    return (
                        <div
                            key={asset.id}
                            className="grid grid-cols-[1fr_auto_auto] items-center gap-4 border-b border-white/5 px-4 py-4 last:border-0 sm:grid-cols-[1fr_150px_110px_80px]"
                        >
                            <Link to={`/market/${asset.id}`} className="min-w-0">
                                <b className="block truncate text-white">{asset.name}</b>
                                <span className="text-xs text-slate-400">
                                    {asset.symbol} · {asset.category}
                                </span>
                            </Link>
                            <b>{formatRupiah(price?.price ?? 0)}</b>
                            <span
                                className={
                            ...
                            <button
                                ...
                                className={`hidden rounded-lg p-2 sm:block ${watched ? "text-amber-300" : "text-slate-500"}`}
                            >
